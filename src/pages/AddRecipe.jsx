...
const AddRecipe = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append("title", title);
    formData.append("description", description);
    formData.append("ingredients", JSON.stringify(ingredients.filter(i => i)));
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 bg-white rounded-2xl shadow-lg mt-10">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <button type="submit" disabled={loading} className="w-full py-3 bg-green-600 text-white rounded-lg font-semibold mt-4">
          {loading ? `Uploading... ${progress}%` : "Add Recipe"}
        </button>
      </form>
    </div>
  );
